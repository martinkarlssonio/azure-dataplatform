...
function Process-Image {
    ...
    if ($images -contains $combinedName) {
        ...
        Copy-Item -Path "..\output\stacc_key" -Destination "$imageName\\stacc_key" -Recurse -Force
        Copy-Item -Path "..\output\coreOutput.json" -Destination "$imageName\\coreOutput.json" -Force
        Start-Sleep -Seconds 5
        ...
        Remove-Item "stacc_key" -Recurse -Force
        Remove-Item "coreOutput.json" -Recurse -Force
        Set-Location ..
    }
}
